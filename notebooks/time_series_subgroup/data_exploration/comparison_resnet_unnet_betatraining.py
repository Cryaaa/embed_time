...
# %%
dataframe_unet = pd.DataFrame(np.array(latents_unet.squeeze()),columns = [f"LD_mu_{i+1}" for i in range(np.array(latents_unet.squeeze()).shape[1])])
dataframe_unet.to_csv(Path(tabular_data)/"20240901_UNet_20z_Latent_Space_OldNorm.csv")
pd.DataFrame({
    "Label":labels_unet,
    "Time":timepoints_unet,
}).to_csv(Path(tabular_data)/"20240901_UNet_20z_Latent_Space_OldNorm_Annotations.csv")
dataframe_unet
# %%
pca_transformer = PCA(n_components = components)
pca_out = pca_transformer.fit_transform(dataframe_unet)
...
dataframe.to_csv(Path(tabular_data)/"20240901_Resnet_26000z_Latent_Space_OldNorm.csv")
pd.DataFrame({
    "Label":labels,
    "Time":timepoints,
}).to_csv(Path(tabular_data)/"20240901_Resnet_26000z_Latent_Space_OldNorm_Annotations.csv")
dataframe
# %%
pca_transformer = PCA(n_components = components)
pca_out = pca_transformer.fit_transform(dataframe)
